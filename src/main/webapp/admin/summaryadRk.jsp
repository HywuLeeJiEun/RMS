	...
	<div class="container">
		<div class="row">
			<table id="bbsTable" class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				<thead>
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;" data-toggle="tooltip" data-html="true" data-placement="right" title="승인 후, <br>pptx로 출력할 수 있습니다."onclick="sortTable(5)">승인<input type="hidden" readonly id="5" style="border:none; width:18px; background-color:transparent;" value=""></input></th>
					</tr>
				</thead>
				<tbody>
					<%
					...
					<tr>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="/RMS/admin/summaryadRkUpdate.jsp?rms_dl=<%= dl %>" data-toggle="tooltip" data-html="true" data-placement="bottom" title="미승인 상태인 경우, 수정 및 삭제가 가능합니다.">
						...
						<% if((dldate.after(today) || dldate.equals(today))  && getSign.equals("미승인")) { %>
							<a class="btn btn-success" style="font-size:12px" href="/RMS/admin/action/summaryadsignOnAction.jsp?rms_dl=<%= dl %>" onclick="return confirm('승인하시겠습니까?\n승인시, 수정이 불가합니다.')"> 승인 </a>
			...
			<%
				if(pageNumber != 1){
			%>
				<a href="/RMS/admin/summaryadRk.jsp?pageNumber=<%=pageNumber - 1 %>"
			...
			<!-- <a href="/RMS/admin/summaryadAdmin.jsp" style="width:50px;" class="btn btn-info pull-right form-control" data-toggle="tooltip" data-placement="bottom" title="[ERP/WEB] 요약본 작성" id="summary"> 작성 </a> -->
		</div>
	</div>
	<!-- 게시판 메인 페이지 영역 끝 -->
